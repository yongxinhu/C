...
write_machine_code:
	...
	# change this part.
	addiu $sp, $sp, 32
	sw $s6, 28($sp)   # $s6 = MIPS instruction 32 bits
	sw $s5, 24($sp)
	sw $s0, 20($sp)
	sw $s1, 16($sp)
	sw $s2, 12($sp)
...
write_machine_code_next_inst:
	...
	move $a0, $v1
	li $a1, 16
	jal parse_int      #$v0 = MIPS instruction number
...
write_machine_code_to_file:
	...
	# YOUR_INSTRUCTIONS_HERE 
	move $a0, $s6
	move $a1, $s0     # output file pointer as character buffer to write into (check it later)
	jal hex_to_str
	# 7. Increment the byte offset by the appropriate amount:
	# YOUR_INSTRUCTIONS_HERE
	addiu $s5 $s5 4    # add 4 to byte offset
