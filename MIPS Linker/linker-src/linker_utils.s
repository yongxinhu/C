...
inst_needs_relocation:
	...
	li $t2,0x2
	li $t3,0x3
	beq $t1,$t2,inst_needs_relocation_need
	beq $t1,$t3,inst_needs_relocation_need
	move $v0,$0	#otherwise
	j inst_needs_relocation_return
inst_needs_relocation_need:
	addiu $v0, $0, 1      #need relocation
inst_needs_relocation_return:
	jr $ra
...
relocate_inst:
	...
	move $s2,$a2
	move $s3,$a3
	move $a0,$s3  #a0 = reloc, a1= byte offset of the instruction in the current file
	jal symbol_for_addr #call function to find addr in reloc table
	beq $v0,$0,relocate_inst_error  #the addr is not in the relocation table
	move $a0,$s2
	move $a1,$v0
	jal addr_for_symbol
	beq $v0,-1,relocate_inst_error
	srl $s0,$s0,25
	sll $s0,$s0,25 # optional(check it later)
	sll $t0,$v0,4
	srl $t0,$t0,6
	OR  $v0,$s0,$t0
	j relocate_inst_return
