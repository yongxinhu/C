...
#push $ra $s1 $s2 $s3 into the stack
.macro push_registers()
	...
	sw $s2 12($sp)
.end_macro
#pop $ra $s1 $s2 $s3 from the stack
.macro pop_registers()
	lw $ra 0($sp)
	lw $s0 4($sp)
	lw $s1 8($sp)
...
addr_for_symbol:
	# YOUR CODE HERE
	push_registers()
	bne $a0 $0 do_work_addr_for_symbol # if (a0 == NULL) 
		li $v0 -1                                       #return -1
		j end_addr_for_symbol
	#---------------------------------------
	do_work_addr_for_symbol:
	move $s0 $a0 #s0 = symbolList
	move $s1 $a1 #s1 = name to look for
	#---------call streq()--------------
	move $a0 $s1 #a0 = name to look for
	lw $a1 4($s0)  #a1 = name of this entry
	jal streq
	bne $v0 $0  recursion_addr_for_symbol  #if name is found
		lw $v0 0($s0)
		j end_addr_for_symbol
	recursion_addr_for_symbol: #else
		lw $a0 8($s0) #a0 = symbolList->next
		move $a1 $s1 #a1 = name to look for
		jal addr_for_symbol
	#---------------------------------------------
	end_addr_for_symbol:
	pop_registers()
