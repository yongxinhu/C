...
SymbolTable* create_table(int mode) {
    ...
    st->len = 0;
    st->mode = mode;
    st->tbl = (Symbol *) calloc(1,INITIAL_SIZE*sizeof(Symbol));
    if(st->tbl == NULL) {
        free(st);
        allocation_failed();
    }
...
int add_to_table(SymbolTable* table, const char* name, uint32_t addr) {
    ...
    if(table->cap == table->len) {
        int newCap = table->cap*SCALING_FACTOR;
        Symbol *newTbl = (Symbol *)calloc(newCap,sizeof(Symbol));
        if(newTbl == NULL) {
            free_table(table);
            allocation_failed();
        }
        for(int i = 0;i<table->len;i++) {
            newTbl[i] = table->tbl[i];
        }
        table->cap = newCap;
        free(table->tbl);
        table->tbl = newTbl;
