...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        if(token == NULL)
            continue;
        int errnum = add_if_label(input_line, token, byte_offset,symtbl);
        if (errnum == -1){
            num = -1;
        ...
        char* args[MAX_ARGS];
        int num_args = 0;
        int errnum2 = parse_args(input_line, args, &num_args);
        if (errnum2 == -1){
            num = -1;
            ret_code = -1;
        }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        int num = translate_inst(output, name, args, num_args,byte_offset, symtbl, reltbl);
        if(num == -1){
