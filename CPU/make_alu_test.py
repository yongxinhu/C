...
name = user_input
shutil.copyfile("alu-harness.circ", "tests-3-1/" + name + ".circ")
f_X = open("tests-3-1/"+name+".X", 'w')
f_Y = open("tests-3-1/"+name+".Y", 'w')
f_S = open("tests-3-1/"+name+".S", 'w')
f_out = open("tests-3-1/reference_output/"+name+".out", 'w')
f_X.write("v2.0 raw\n")
f_Y.write("v2.0 raw\n")
f_S.write("v2.0 raw\n")
print("Created files "+name+".X, .Y, and .S in your tests folder, and "+name+".out in your tests-3-1/reference_output folder.")
...
for t in range(cycles):
    ...
    #S
    while success==0:
        user_input = raw_input("Time "+str(t)+"\tFunction (e.g add, slt): ")
        if user_input == 'mult':
            result2 = 1
        else:
            result2 = 0
